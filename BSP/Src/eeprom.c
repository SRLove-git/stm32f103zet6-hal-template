...
HAL_StatusTypeDef EEPROM_ReadBuffer(uint16_t addr, uint8_t *buf, uint16_t len)
{
    ...
    return HAL_I2C_Mem_Read(&hi2c1, EEPROM_ADDR, addr, I2C_MEMADD_SIZE_8BIT,
...
HAL_StatusTypeDef EEPROM_WriteBuffer(uint16_t addr, const uint8_t *buf, uint16_t len)
{
    ...
    while (len > 0U)
    {
        ...
        ret = HAL_I2C_Mem_Write(&hi2c1, EEPROM_ADDR, addr, I2C_MEMADD_SIZE_8BIT,
                                (uint8_t *)buf, chunk, EEPROM_TIMEOUT);
        if (ret != HAL_OK)
        {
            break;
        }
