...
| `crc.h` | CRC-8/SMBUS、CRC-16/Modbus、CRC-32/IEEE | `CRC8_Compute` / `CRC16_Modbus` / `CRC32_Compute` |
...
开机自检结果会被主程序复用（MPU6050/LCD 不重复初始化），启动更快；
删除 `BSP_SelfTest()` 调用时演示逻辑会自动补做初始化。
